...
function initSales() {
    document.getElementById('sale-btn')?.addEventListener('click', () => {
        ...
        if (!tileId || qty <= 0) return UI.toast('Выберите товар и количество!', 'error');
        ...
        fetch('/api/sales', {
            ...
            if (res.ok) {
                UI.toast('✅ Продажа успешно оформлена!', 'success'); // Замена alert
                loadTable();
            } else {
                UI.toast('Ошибка при оформлении продажи', 'error');
            }
        });
    });
...
function loadProducts() {
    fetch('/api/products').then(res => res.json()).then(data => {
        ...
        if(!s2) return; // Защита от отсутствия элемента
