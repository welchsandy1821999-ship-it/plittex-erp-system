...
const EXCLUDE_CATEGORIES = new Set([
    ...
    // 'Ввод начальных остатков' — намеренно НЕ исключается: такие записи создаются
    // именно для закрытия взаиморасчётов и должны распределяться по заказам.
    // 'Корректировка Баланса' — аналогично.
    'Корректировка долга',
    'Взаимозачет',
    'Взаимозачет аванса',
    'Перевод',
