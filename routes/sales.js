...
module.exports = function(pool, getWhId, getNextDocNumber) {
        ...
        try {
            ...
            if (refundAmountNum > 0) {
                if (refund_method === 'cash' && account_id) {
                    ...
                    await client.query(`UPDATE accounts SET balance = balance - $1 WHERE id = $2`, [refundAmountNum, account_id]);
    ...
    // Оформление заказа (твой полный старый код)
    router.post('/api/sales/checkout', async (req, res) => {
        ...
        try {
            ...
            const docNum = await getNextDocNumber(client, 'ЗК', 'client_orders', 'doc_number');
            ...
            const finalAmount = Number(finalAmountBig.round(2));
            ...
            if (payment_method === 'debt') {
                ...
                if (advNum > 0) {
                    ...
                    await client.query(`UPDATE accounts SET balance = balance + $1 WHERE id = $2`, [advNum, account_id]);
                }
    ...
    // --- ОСТАЛЬНЫЕ МАРШРУТЫ ПРОДАЖ И АНАЛИТИКИ (СОКРАЩЕННО, ПРОСТО ПЕРЕНЕСЕНЫ КАК ЕСТЬ) ---
    ...
    router.delete('/api/sales/orders/:id', async (req, res) => {
        ...
        try {
            ...
            if (palletsToReturn > 0) await client.query(`UPDATE counterparties SET pallets_balance = GREATEST(pallets_balance - $1, 0) WHERE id = $2`, [palletsToReturn, order.counterparty_id]);
    ...
    router.post('/api/sales/orders/:id/ship', async (req, res) => {
        const orderId = req.params.id; const { items_to_ship, driver, auto, poa_info, pallets, user_id } = req.body; const client = await pool.connect();
        try {
            await client.query('BEGIN');
            const docNum = await getNextDocNumber(client, 'УТ', 'inventory_movements', 'description');
            ...
            for (let row of checkRes.rows) { if (parseFloat(row.qty_shipped) < parseFloat(row.qty_ordered)) { allCompleted = false; break; } }
    ...
    router.delete('/api/sales/shipment/:docNum', async (req, res) => {
        const docNum = req.params.docNum; const client = await pool.connect();
        try {
            await client.query('BEGIN');
