...
- **Имя файла:** `erp-backup-YYYY-MM-DD_HH-mm-ss.backup` (локальная дата и время сервера). При коллизии в ту же секунду добавляется суффикс `_1`, `_2` и т.д. Старые файлы вида `erp-backup-YYYY-MM-DD.backup` по-прежнему допускаются для скачивания. За один день можно создать неограниченное число бэкапов.
...
- После проведения распалубки в `POST /api/move-wip` статус партии задаётся **тем же правилом** (расчёт `drying_balance` по сушилке), без разовых апдейтов `status='completed'` в обход формулы.

### Новые типы движений (`inventory_movements.movement_type`) при принудительном закрытии партии (`isComplete` в `/api/move-wip`)

| Код в БД | Назначение |
|----------|-------------|
| `tech_loss_receipt` | Если фактический остаток сырца в сушилке **больше** суммы введённых выходов (1 сорт / 2 сорт / бой): положительная дельта **оприходуется как тех.потери** на склад с типом `defect` (в UI — Склад №6 / утиль); та же сумма затем включается в списание `wip_expense`, баланс сушилки сходится к нулю. |
| `wip_adjustment_receipt` | Если сумма выходов **больше** остатка в сушилке: недостающее сырьё **дооприходуется на сушилку** (`warehouse=drying`), после чего строка `wip_expense` списывает согласованный объём; баланс сушилки сходится к нулю. |

Обе строки выполняются **внутри транзакции** распалубки и связаны общим `transaction_id` с остальной группой движений.

### Правило себестоимости единицы 1-го сорта (бизнес-согласование)

Для финансовой нагрузки партии делитель — **строго фактический выпуск 1 сорта** поля `production_batches.actual_good_qty` (накопление по операциям распалубки), **не** `planned_quantity`. В частности:

- себестоимость «на единицу» материалов + амортизации в контрактах, где выполняется такое деление, использует `actual_good_qty` (fallback на `planned_quantity` для расчётов здесь убран);
- аналитика продаж (`utils/salesAnalyticsUnitCost.js`): амортизация на исторических завершённых партиях нормализуется по `actual_good_qty` только.

Схема БД и полный каталог ENUM/ограничений по `movement_type` — **`.antigravity/db_protocol.md`** (новые коды нужно добавить там при миграции схемы, если они зафиксированы ограничителем).
