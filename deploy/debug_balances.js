...
conn.on('ready', async () => {
    // Проверяем, есть ли сотрудники с отрицательным prev_balance
    console.log('=== Checking salary balances for June 2026 ===\n');
    
    await exec(conn, Q(`
        SELECT e.id, e.full_name, e.status, e.exclude_from_salary,
               COALESCE(
                   (SELECT SUM(CASE 
